...
def train_medal_prediction_model_with_features(df, model_choice="Random Forest"):
    df = df.dropna(
        subset=['Age', 'Height', 'Weight', 'Sport', 'Event', 'Sex', 'Medal', 'Team']
    ).copy()
    ...
    df['Previous_Medals'] = (
        df.groupby('Name')['Medal']
        .transform(lambda x: x.notna().cumsum() - 1)
    )

    df['Previous_Medals'] = df['Previous_Medals'].clip(lower=0)
    ...
    # Features and target
    features = ['Age', 'Height', 'Weight', 'BMI', 'Previous_Medals', 'Sex', 'Sport', 'Event', 'Team']
    X = df[features]
    y = df['Medal']
    ...
    if hasattr(model, "feature_importances_"):
        ...
        feature_importance = np.zeros(len(features))  # fallback
    ...
    return model, encoders, feature_importance, acc
